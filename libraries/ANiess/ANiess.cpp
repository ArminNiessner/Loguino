...
uint8_t ANiess::b_seek(unsigned long pos) {
	_pos = pos;
	return seek(pos);
}
uint8_t ANiess::b_write(const uint8_t b) {
	_res = write(b);
	_pos++;
	if (_pos == _max_length) {
		b_seek(0);
...
uint8_t ANiess::b_write(const uint8_t *b, uint8_t length) {
...
int ANiess::b_read(uint8_t *dest, int length) {
...
unsigned long ANiess::available(void) {
	return ((unsigned long) (_max_length + _write_pos - _read_pos))
			% _max_length;
}

uint8_t ANiess::freeSpace(uint8_t length) {
	if (_end == _write_pos)
...
int ANiess::readPacket(uint8_t *dest) {
	return b_read(dest, _packet_length);
}

uint8_t ANiess::readBinary(unsigned long pos, uint8_t length,
		uint8_t *dest) {
	b_seek(pos);
	return b_read(dest, length);
}

uint8_t ANiess::readBinary(unsigned long pos, unsigned long stop,
...
void ANiess::set(unsigned long pos) {
#if SERIAL_DEBUG
	Serial.print("Set: ");
	Serial.println(pos);
#endif
...
void ANiess::seekReadPointer(unsigned long pos) {
	_read_pos = pos;
}

void ANiess::reset(void) {
	set(0);
	resetStorage();
}

uint8_t ANiess::initNextPacket(void) {
	return initPacket(_read_pos);
}

uint8_t ANiess::initPacket(unsigned long pos) {
...
uint8_t ANiess::addPacket(const uint8_t *payload, uint8_t length) {
