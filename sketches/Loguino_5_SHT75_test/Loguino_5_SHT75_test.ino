...
#include <Sensirion.h>                    // lib for SHT75 sensor
...
const uint8_t dataPin  =  9;            // SATA of SHT75
const uint8_t clockPin =  7;            // SCK of SHT75
...
//  delay(50);
  ...
  tempSensor.measure(&temp, &rh, &dew); // read SHT75 sensor
  ...
  if (Filesize == 0 && dataFile) {
    dataFile.println("Date,temp,rh,dew,battV");         // if File is empty (new), create header (if appropiate add "dd2" or "xyl2")
  } else {
