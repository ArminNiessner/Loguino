...
/*
 * Example sketch to take measurements of four thermocouple "Granier-sap-flow" sensors. 
 * 
...
char str_xyl2[10] = "";
float xyl3;                             // xylem sensor #3
char str_xyl3[10] = "";
float xyl4;                             // xylem sensor #4
char str_xyl4[10] = "";
...
void measure() {
  ...
  // meassure xyl #1
  mcp342x.setConf(addr, 1, 0, mode, rate, 3); // (gain = 3)
  delay(300);
  ...
  // meassure xyl #2
  mcp342x.setConf(addr, 1, 1, mode, rate, 3); // (gain = 3)
  delay(300);
  ...
  // meassure xyl #3
  mcp342x.setConf(addr, 1, 2, mode, rate, 3); // (gain = 3)
  delay(300);
  xyl3 = mcp342x.getData(addr) * 1000000 / 40;    // convert µV in deltaT
  dtostrf(xyl3, 5, 4, str_xyl3);
  delay(50);

  // meassure xyl #4
  mcp342x.setConf(addr, 1, 3, mode, rate, 3); // (gain = 3)
  delay(300);
  xyl3 = mcp342x.getData(addr) * 1000000 / 40;    // convert µV in deltaT
  dtostrf(xyl3, 5, 4, str_xyl3);
  delay(50);
  ...
  if (Filesize == 0 && dataFile) {
    dataFile.println("Date,xyl1,xyl2,xyl3,xyl4,battV");         // if File is empty (new), create header
  } else { 
  }
  if (dataFile) {
    ...
    dataFile.print(",");
    dataFile.print(str_xyl3);
    dataFile.print(",");
    dataFile.print(str_xyl4);
    dataFile.print(",");
  ...
  Serial.print(",");
  Serial.print(str_xyl3);
  Serial.print(",");
  Serial.print(str_xyl4);
  Serial.print(",");
