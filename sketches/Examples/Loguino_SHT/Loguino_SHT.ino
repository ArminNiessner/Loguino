...
/*
 * Example sketch to take measurements of a SHT75 temperature and humidity sensor. 
 * 
...
const int enable_temp = 6;              // pin to enable power supply to SHT
...
void measure() {
  ...
  digitalWrite(enable_temp, HIGH);        // enable voltage supply to SHT
  delay(50);

  tempSensor.measure(&sht_temp, &sht_rh, &sht_dew);
  ...
  digitalWrite(enable_temp,LOW);          // disable voltage supply to SHT
  ...
  if (Filesize == 0 && dataFile) {
    dataFile.println("Date,sht_temp,sht_rh,sht_dew,battV");         // if File is empty (new), create header
  } else { 
  }
  if (dataFile) {
    ...
    dataFile.print(",");
    dataFile.print(sht_temp);
    dataFile.print(",");
    dataFile.print(sht_rh);
    dataFile.print(",");
    dataFile.print(sht_dew);
    dataFile.print(",");
