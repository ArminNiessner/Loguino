...
/*
 * Example sketch to take measurements of a DHT22 temperature and humidity sensor. 
 * 
...
const int enable_temp = 6;              // pin to enable power supply to DHT
...
void measure() {
  ...
  digitalWrite(enable_temp, HIGH);        // enable voltage supply to DHT

  delay(50);

  dht_temp = dht.readTemperature();
  dht_rh = dht.readHumidity();
  ...
  if (Filesize == 0 && dataFile) {
    dataFile.println("Date,dht_temp,dht_rh,battV");         // if File is empty (new), create header
  } else { 
  }
  if (dataFile) {
    ...
    dataFile.print(",");
    dataFile.print(dht_temp);
    dataFile.print(",");
    dataFile.print(dht_rh);
    dataFile.print(",");
